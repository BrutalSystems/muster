...
> Shared standard, carried independently by
> [muster](https://github.com/BrutalSystems/muster),
> [tincan](https://github.com/BrutalSystems/tincan) and
> [birddog](https://github.com/BrutalSystems/birddog).
>
> Each repo implements this on its own and keeps its own copy. The copies will
> diverge — that is the consequence of the repos being independent, not a
> defect in it — so this file makes no claim that they are identical. What
> matters is that anything true of only one repo is marked
> `> **Repository-specific, <repo>.**` rather than left inline, because an
> unmarked difference cannot be told from shared standard.
>
> This copy was reconciled with tincan at `5cdcb4e` on 2026-09-22. Deliberate
> muster differences since: the fork guard has its own heading, and the
> `EXCEPTIONS` example names muster's fixture.
>
> Statements here describe the past or the standard itself, never what is in
> flight elsewhere. A standing claim about another repo rots into a falsehood
> the moment that repo moves; a dated fact merely ages, and ages legibly.

No secrets, tokens, session IDs, or private paths belong in this file or in the
workflows it describes — each repo is public.
...
- **`ci.yml`** runs two things. A Node matrix covering the `engines` floor and
  the current major above it runs build, typecheck, test and any repo-specific
  static gates — the exact step list is not pinned, since repos genuinely
  differ, but no repo skips build and test. A **separate job** pins npm and runs the
  same tarball verification `publish.yml` uses, as an early
  warning rather than a separate inline check, so the two workflows never drift
  apart on what "verified" means. Packing is split out because it must use the
  npm that publishes, and the matrix must not — pinning that leg would stop it
  exercising the npm a Node-floor user actually has. **No contract suite, no
  publish steps, no credentials.**
- **`publish.yml`** runs only on a `v*.*.*` tag. It is the sole path that
...
a tag". npm says exactly that when trust is established: _"anyone with GitHub
repository write access can publish."_
...
file can ship under one npm and be omitted by another _on the same commit_,
...
The publish job is gated so it runs only in the canonical repo, never a fork:

```yaml
if: github.repository == 'BrutalSystems/<repo>'
```

It sits on the job itself, before any of the order below.

> **Why this is its own heading.** It was a sentence inside "Pipeline order",
> and that is where a duplicated paragraph hid in one copy from 0.5.1 until
> 2026-09-21: the sentence was added by re-pasting the paragraph it belonged to
> and appending to the copy, leaving both. A gate that guards every release
...
5. **Contract or integration suite**, where the repository has one — run
   ...
   > **Repository-specific, muster.** Muster's contract suite runs against an
   > independently installed Tin Can, pinned to the verified baseline in
   > `RELEASING.md` and installed `--no-save` into a temp prefix. It is a test
   > prerequisite and never a dependency: Tin Can appears in no dependency
   > list, the suite skips unless `MUSTER_CONTRACT=1`, and the naming contract
   > it checks is a frozen local fixture rather than something fetched. The
   > baseline is a pin, never a range — a floating version would change what
   > "verified" means without a commit.
...
10. **Confirm the registry serves it.** Poll `npm view`; a publish usually
    lags one to two minutes, but budget ten — five has been exceeded in
    practice. Print `dist.integrity`. This step can fail while the publish
    itself succeeded, so **steps 11 and 12 must not be conditioned on it.** A
    runner skips every later step once one fails, and a version sitting on the
    registry with no Release and no artifact check is far worse than a slow
    poll. Condition them on the publish's own outcome instead.
11. **Verify the published artifact matches what was tested.** Download it
    ...
    tested at step 8 — _not_ a re-run of step 7's rules, since two different
    compliant lists could both pass those. Skip, do not fail, if the registry
    has not replicated the tarball after retries: the publish itself already
    succeeded.
12. **Create the GitHub Release.** Mandatory — which means conditioned on the
    package being on the registry, whether just published or already there on
    a re-run, and never on the run being green to that point. From the release
    notes if the repository has them, otherwise `gh release create
--notes-from-tag`.
...
is mostly a formality, since `files` _is_ an allowlist. It earns its keep on
...
by definition — the widened entry _is_ the allowlist — and only a denylist
notices.
...
> **Repository-specific, muster.** Muster has exactly one:
> `test/fixtures/canonical-id.json`, the frozen naming fixture, which ships
> because consumers implement against it.
...
1. Bump the version everywhere the repository keeps it. Prefer a test that
   ...
   > **Repository-specific, muster.** `npm version` stamps the `## Unreleased`
   > section of `RELEASE_NOTES.md` into `## <version> — <date>` and stages it,
   > so the notes land in the version commit. The stamp refuses rather than
   > inventing content: a missing or empty section fails before a tag exists,
   > which is recoverable, instead of failing in CI after it, which is not.
